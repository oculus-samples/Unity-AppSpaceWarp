...
public class OVRLint : EditorWindow
{
	...
	[MenuItem("Oculus/Tools/OVR Performance && Compatibility Checker")]
	static void Init()
	{
		// Get existing open window or if none, make a new one:
		EditorWindow.GetWindow(typeof(OVRLint), false, "OVR Performance & Compatibility Checker");
	...
	void OnGUI()
	{
		GUILayout.Label("The OVR Performance & Compatibility Checker identifies common issues in your project configuration and assets, which can cause errors and slowdown in your application.", EditorStyles.wordWrappedLabel);
...
#if !(USING_XR_SDK && UNITY_2019_3_OR_NEWER)
		// Generate error when Vulkan is selected as the preferred graphics API and project is not using the XR Plugin Management package
		if (!PlayerSettings.GetUseDefaultGraphicsAPIs(BuildTarget.Android))
		{
			GraphicsDeviceType[] apis = PlayerSettings.GetGraphicsAPIs(BuildTarget.Android);
			if (apis.Length >= 1 && apis[0] == GraphicsDeviceType.Vulkan)
			{
				AddFix(eRecordType.StaticAndroid, "Use XR Plugin Management Package", "In order to use Vulkan with an XR project, apps must install the 'XR Plugin Management' package in Project Settings, and configure it with at least one Plug-in Provider.", delegate (UnityEngine.Object obj, bool last, int selected)
				{
					EditorApplication.ExecuteMenuItem("Edit/Project Settings...");
				}, null, false, "Open Settings");
			}
		}
#endif
